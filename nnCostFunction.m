...
for i=1:num_labels
    one=ones(m,1);
    J = J -sum(log(hx(:,i)).*Y(:,i)+log(one-hx(:,i)).*(one-Y(:,i)))/m;
end
...
for i=2:(input_layer_size+1)
    sum_theta1=sum_theta1+Theta1(:,i)'*Theta1(:,i);
end

for i=2:(hidden_layer_size+1)
    sum_theta2=sum_theta2+Theta2(:,i)'*Theta2(:,i);
end
...
for i=1:m
    ...
    d2 = Theta2(:,2:(hidden_layer_size + 1))' *d3' .*sG(i,:)';
